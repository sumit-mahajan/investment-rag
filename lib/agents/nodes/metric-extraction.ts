...
import { getGeminiExtractionModel } from "../gemini";
...
/** Gemini structured output rejects Zod `.nullable()` (type arrays). Use sentinels instead. */
const metricExtractionSchema = z.object({
  currency: z
    .enum(["USD", "INR", "EUR", "GBP", "OTHER", "UNKNOWN"])
    .describe("Currency implied by the table/header. UNKNOWN if not determinable."),
  unitFromTable: z
    .string()
    .describe(
      'Exact unit phrase from the filing e.g. "in millions", "in crore". Empty string if not found.'
    ),
  figure: z
    .string()
    .describe("Primary numeric result as stated (include commas; do not convert units). Empty string if not found."),
  yoyChange: z
    .string()
    .describe('YoY % or period comparison if present e.g. "+4.7% YoY". Empty string if not found.'),
  displayLine: z
    .string()
    .describe(
      "One-line human-readable value with correct currency and unit. Empty string if not found."
    ),
});
...
function formatChunksForPrompt(
  ...
  // Capped at 6 chunks × 900 chars to keep extraction prompts focused.
...
- If currency/unit is unclear, use currency UNKNOWN and empty strings — do not guess USD.
...
**Debt/Equity Ratio:** Compute total debt ÷ total shareholders' equity from balance sheet (same currency). Show the ratio (e.g. "0.05" or "5%") with debt and equity amounts. Use empty strings if totals are missing — never output 0.0 unless the filing explicitly shows zero debt.
**Forward guidance:** empty unless explicit forward-looking numeric guidance appears (rare in 10-K/annual reports).
...
Return empty strings (and currency UNKNOWN) only if the metric is absent from every passage.`;

function parseExtraction(raw: z.infer<typeof metricExtractionSchema>): MetricExtractionFields {
  return {
    currency: raw.currency === "UNKNOWN" ? null : raw.currency,
    unitFromTable: normalizeMetricValue(raw.unitFromTable),
    figure: normalizeMetricValue(raw.figure),
    yoyChange: normalizeMetricValue(raw.yoyChange),
    displayLine: normalizeMetricValue(raw.displayLine),
  };
}
...
${formatChunksForPrompt(chunks)}`;
  ...
  try {
    ...
    if (isGeminiRateLimitError(error)) {
      console.error(`Gemini rate limit during metric extraction (${metric.label})`);
...
export async function metricExtractionNode(
  ...
  const model = getGeminiExtractionModel().withStructuredOutput(metricExtractionSchema);
